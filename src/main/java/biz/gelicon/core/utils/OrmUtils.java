...
public class OrmUtils {
    ...
    public static Field getField(Class cls,String colName) {
        ...
        return fields.stream()
                .filter(f -> f.isAnnotationPresent(Column.class))
                .filter(f->f.getAnnotation(Column.class).name().equals(colName))
                .findFirst()
                .orElse(null);
    }
    ...
    public static String prepareSQLText(String sqlText, String mainAlias,String predicate,TableMetadata tableMetadata,Pageable page) {
        ...
        if (page != null) {
            ...
            if (limit != null) {
                sqlText = DialectFactory.getDialect().buildSqlTextWithLimit(
                        sqlText,
                        page.getPageSize(),
                        page.getPageNumber()
                );
    ...
    public static void logSQL(String sqlText) {
        Boolean flag = false;
        try {
            flag = (Boolean) MainApplication.getApplicationContext()
                    .getBean("getShowSQLFlag");
        } catch (Exception e) {
        }
        if (flag) {
