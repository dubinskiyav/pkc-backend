...
public class Query<T> {
    ...
    public static class QueryBuilder<T> {
        ...
        public QueryBuilder<T> injectSQLIf(boolean cond, String tag, String thenClause,String elseClause) {
            if(cond) {
                injects.put(tag,thenClause);
            } else {
                injects.put(tag,elseClause);
            }
            return this;
        }
        ...
        public Query<T> build(Class<T> cls) {
            ...
            if(page!=null) {
                ...
                if(page.getPageSize()<Integer.MAX_VALUE) {
                    sql = DialectFactory.getDialect().buildSqlTextWithLimit(
                            sql,
                            page.getPageSize(),
                            page.getPageNumber()
                    );
